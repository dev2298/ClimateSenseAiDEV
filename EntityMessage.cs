...
namespace NavCommonModuleBLL.MDM.Main2
{
    public class EntityMessage : Entity
    {
        ...
        private static bool rollback_flag = false;
            ...
            lock (_entityMessageLock)
            {
                try
                {
                    ...
                    if (entityData != null)
                    {
                        ...
                        SaveClientEntitydata(entityData);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void SaveClientEntitydata(PublishEntity entityData)
        {
            var startDateTime = DateTime.UtcNow;
            ...
            if (clienIds.Any())
            {
                ...
                var rollbackconfig = entityConfig.GetSystemConfigurationData(clientIds, "isRollbackEnabled");
                Dictionary<int, bool> isRollbackEnabled = rollbackconfig.ToDictionary(c => c.ClientId, c => c.Value == "true");
                ...
                Parallel.ForEach(clienIds, (clientId) =>
                {
                    if (!ismasterDecouplingApplicable.ContainsKey(clientId) || !ismasterDecouplingApplicable[clientId])
                    {
                        ...
                        try
                        {
                            ...
                            if (!isRollbackEnabled.ContainsKey(clientId) || !isRollbackEnabled[clientId])
                            {

                                ExecuteWithRetry(
                                () => EntityDAL.SyncDataToClient(primaryOperationId, clientId),// Main action
                                () => EntityDAL.RollbackTransaction(primaryOperationId),         // Rollback action
                                primaryOperationId
                                );
                            }
                            else
                            {
                                EntityDAL.ProcessEntity(objSaveEntity, primaryOperationId, clientId);
                    ...
                    else
                    {
                        ...
                        if (client != null)
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    EntityDAL.ProcessEntityDecoupled(objSaveEntity, jsonSysInfo, transactionalClientID, primaryOperationId,clientId);
